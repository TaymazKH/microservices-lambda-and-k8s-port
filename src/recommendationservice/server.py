...
def handle_request(msg, req_data):
    return Service.ListRecommendations(msg)
...
def decode_request(req_data):
    ...
    if req_data['requestContext']['http']['path'] == f"/{RECOMMENDATION_SERVICE}/{LIST_RECOMMENDATIONS_RPC}":
        msg = pb.ListRecommendationsRequest()
    else:
